...
\begin{document}
	...
	\section{Les mathématiques de la transformation}
		...
		Soit donc $\mathcal{T}$ toutes les transformations possibles, c'est à dire les fonctions de $\bar{\mathcal{C}}$ dans $\bar{\mathcal{C}}$. Une transformation $f$ est alors valide si $\rho(c) \implies \rho(f(c))$. 
		
		A ce niveau là, nous avons une logique d'invariant, de configurations et de transformations. Notre hypothèse fondamentale consiste à considérer que les transformations possibles sont une combinaison de transformations élémentaires. Dans le cas du graphe, il s'agissait des événements. A ce niveau de généralité, nous devons le postuler. 
		
		\begin{postulat}[Postulat de famille engendrant les transformations]
			Pour tout système, il existe une famille de transformations unitaires, élémentaires, telle que : 
			\begin{itemize}
				\item toute transformation est la combinaison finie de ces transformations unitaires.
				\item toute transformation valide est la combinaison finie de transformations unitaires valides. 
			\end{itemize} 
			
			Il n'est pas nécessaire que la famille des transformations soit finie, ou que les transformations unitaires forment un ensemble fini. On impose juste l'équivalent d'une famille génératrice et une composition finie. 
		\end{postulat}

		On peut alors attacher un coût à une transformation $f$, sous forme d'entier, c'est  à dire le nombre d'actions unitaires nécessaires. 
		
		\begin{definition}
			Un coût associe un entier $n \in \mathbb{N}$ à une transformation valide (passant d'une configuration valide à une autre valide). Une transformation unitaire a un coût de $1$. Une transformation est la composée finie de transformations unitaires. La décomposition n'est pas nécessairement unique. Nous prenons le coût de la transformation comme le nombre minimal de transformations unitaires nécessaires pour produire la transformation. 
		\end{definition}
		
		C'est alors l'occasion de mesurer une distance entre deux configurations comme le nombre minimum de transformations unitaires qu'il suffit d'appliquer pour passer de l'une à l'autre. La notion de distance, cependant, serait a priori inadaptée : on ne sait pas en général assurer la symétrie ou l'inégalité triangulaire. Ce qui nous amène à une notion d'écart pour ne pas parler de distance. 
		
		\begin{definition}
			Pour deux configurations données $c$ et $c'$, on appelle écart d'édition le nombre de transformations unitaires minimal à appliquer pour passer de $c$ à $c'$. Par définition, l'écart entre deux configurations identiques est $0$. 
		\end{definition}
		
		Nous avons tous les outils conceptuels pour parler de la notion d'irréversibilité. Ce qui a distingué l'inertie des événements est au fond le coût théorique de revenir à la configuration initiale. 
		
		\begin{definition}
			Toute transformation unitaire $t$ passant de $c$ à $c'$ telle que le passage de $c'$ à $c$ ne soit pas valide ou à un coût supérieur est dite irréversible. 
		\end{definition}
		
		Cette définition nous permet de proposer une description du modèle, avec ce niveau de généralité, comme un graphe valué. Les sommets sont les configurations et l'indication de leur validité (ou simplement les configurations valides). Pour les liens, plusieurs options sont possibles : 
		\begin{itemize}
			\item \textbf{les liens sont les transformations unitaires}. La représentation permet de montrer les chemins, de comparer les degrés entrants et sortants, etc. C'est une vision locale. Le graphe peut tout à fait être cyclique pour les transformations réversibles. 
			\item \textbf{les liens assemblent les transformations tant qu'il n'y a qu'une possibilité}. La valeur du lien n'est plus seulement une transformation, mais aussi son coût. C'est cette représentation que nous retiendrons. 
		\end{itemize}
		
		Ainsi, nous terminons avec un ensemble $\bar{\mathcal{C}}$ et le prédicat $\rho$, ainsi qu'une fonction $\nu : \bar{\mathcal{C}} \rightarrow \mathfrak{P}(\mathcal{T} \times \mathbb{N})$ donnant les transformations accessibles à chaque configuration et leur cout en transformations unitaires. Il liste, en théorie, les états possibles du système et comment y parvenir. Les conditions de viabilité s'appliquent, de sorte qu'on peut modifier légèrement la définition pour rester dans $\mathcal{C}$. L'information essentielle étant de savoir comment transformer et à quel coût (quitte à tester avec $\rho$ ensuite).
		
		\begin{definition}
			Étant donné un système, une description ontologique consiste à lister ses configurations $\bar{\mathcal{C}}$, le prédicat de validité $\rho$, les transformations $\mathcal{T}$ et leur cout en transformations élémentaires. Dès lors, le résultat produit un graphe, dit \textbf{représentation canonique}, liant les configurations entre elles par des couples de transformations et leur cout. 
		\end{definition}
				
		Cette vue permet de bénéficier de toutes les méthodes d'analyse de graphe.
		
		\begin{itemize}
			\item \textbf{L'attracteur (ponctuel ou sous-graphe)} : c'est une configuration qui présente de très nombreux arcs entrants et quasiment aucun arc sortant. Le système y est "capturé" car la majorité des transformations mène irrémédiablement vers ce macro-état. Par exemple, imaginons un été sans pluie et une plaine couverte d'arbres secs. Quel que soit le point de départ du feu dans la zone, il se propagera et finira par consumer toute la zone. Autrement dit, les chaînes causales convergent toutes vers l'attracteur final de la forêt calcinée.
			
			\item \textbf{Le point d'articulation} : bien que le terme de \og{}goulot d'étranglement\fg{} offre une intuition visuelle, les termes consacrés en théorie des graphes sont le \textit{point d'articulation} (un nœud dont la suppression scinde le réseau) ou l'\textit{isthme} (pour une arête). Dans l'espace des configurations, il est fréquent que le passage d'un sous-ensemble d'états stables à un macro-état d'effondrement soit contraint par un très faible nombre de configurations intermédiaires obligatoires. L'identification de cette \og{}coupure topologique\fg{} offre une assise structurelle à la résilience systémique : il est illusoire de vouloir interdire la combinatoire infinie des événements menant à la crise. Il suffit, en revanche, d'isoler ce point d'articulation invalide. Le graphe canonique s'en trouve déconnecté, isolant structurellement l'attracteur d'effondrement, qui devient dès lors inaccessible.
			
			\item \textbf{L'asymétrie combinatoire (et la flèche du temps)} : l'écart d'édition entre deux configurations n'est pas symétrique. Le passage de $c$ à $c'$ peut résulter d'un événement unique libérant une chaîne causale (coût unitaire de $1$). Le retour de $c'$ vers $c$ requiert, à l'inverse, un enchaînement précis de $N$ événements reconstructifs. Plus $N$ est grand, plus se forme un obstacle purement combinatoire. Depuis la configuration $c'$, l'arbre des configurations accessibles diverge de manière exponentielle : à chaque étape de la reconstruction, de multiples autres événements valides s'offrent au système. L'unique trajectoire permettant de retrouver l'état initial devient statistiquement inaccessible face à la divergence exponentielle des chemins possibles. L'irréversibilité est donc une propriété purement combinatoire liée au volume de l'espace des possibles.
			
			\item \textbf{La contrôlabilité structurelle par entités directrices} : dans leurs travaux fondateurs, Liu, Slotine et Barabási \cite{Controlability} démontrent qu'il est possible de piloter intégralement la dynamique d'un réseau complexe en n'agissant que sur un sous-ensemble restreint de composants, appelés \og{}nœuds directeurs\fg{} (\textit{driver nodes}). Dans notre ontologie, ce théorème illustre remarquablement l'absence d'agentivité du système. Piloter la représentation canonique ne consiste nullement à offrir une trajectoire de facilité que le réseau \og{}choisirait\fg{} de suivre. L'opération, menée par un acteur externe, consiste à imposer délibérément des événements sur ces seules entités directrices. La chaîne causale s'active alors mécaniquement de proche en proche, contraignant le système global à converger vers la configuration visée par le contrôleur.
		\end{itemize}
	...
	\section{Résumé}
		...
		Qu'il nous soit permis de conclure sur une précaution qui nous paraît nécessaire : \textbf{l'illusion de l'agentivité systémique.} Il convient de dissiper ici un biais cognitif fréquent lors de l'observation des dynamiques complexes. Face à une défaillance en cascade, le vocabulaire courant laisse souvent entendre que le système \og{}réagirait\fg{}, \og{}se reconfigurerait\fg{} ou \og{}ferait le choix\fg{} d'une trajectoire pour restaurer son équilibre. Dans notre ontologie, cette vision anthropomorphique est caduque. Le système ne décide rien et n'a aucune conscience de son état global. L'invariant n'est pas un but à atteindre, mais l'expression stricte des dépendances locales. Si une entité critique disparaît, les entités qui en dépendaient structurellement perdent instantanément leurs préconditions d'existence. Elles disparaissent donc à leur tour, propageant cette invalidation de proche en proche. La chaîne causale n'est pas une stratégie de réparation exécutée par le réseau, mais la stricte actualisation mécanique d'un effondrement localisé. À l'image d'une voûte qui s'écroule lorsqu'on en retire la clé, les pierres qui tombent ne cherchent pas à rétablir un équilibre : elles tombent car elles ne sont plus soutenues. De la même manière, la défaillance en cascade s'éteint mécaniquement là où les contraintes locales cessent d'être violées. La validité n'est pas la finalité du mouvement, elle en est le simple constat d'arrêt.
