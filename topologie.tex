...
\section{Une formalisation du modèle : le graphe canonique}

La mécanique fondamentale de l'ontologie postule une logique d'entités évoluant sous l'effet d'événements dictés par des contraintes locales. Jusqu'ici, nous avons décrit cette mécanique intime au niveau microscopique. Cependant, chercher à modéliser mathématiquement chaque état transitoire ou chaque instabilité lors d'une reconfiguration alourdit inutilement le modèle. 

Pour exploiter mathématiquement ce système et en tirer des outils d'analyse macroscopique, nous devons l'essentialiser. L'objectif n'est plus de simuler la mécanique interne de la chute (la \og{}boîte noire\fg{} de la cascade), mais de cartographier l'espace des possibles et la topologie de ses états d'équilibre.

\subsection{Configurations valides et chemins causaux}

Plutôt que de modéliser tous les états imaginables pour les soumettre à un prédicat de validité global, nous postulons directement l'espace géométrique de notre système au repos. 

Soit $\mathcal{C}$ l'ensemble des \textbf{configurations valides}. Une configuration appartient à $\mathcal{C}$ si et seulement si toutes les entités qui la composent voient leurs préconditions d'existence locales respectées. Il s'agit d'un état d'équilibre intrinsèque et purement mécanique.

Sur cet espace agit une famille de transformations unitaires $\mathcal{U}$, correspondant aux événements élémentaires insécables (création ou suppression d'entité ou de lien). 

Lorsqu'un événement déclencheur vient rompre une dépendance locale d'une configuration $c \in \mathcal{C}$, le système quitte son équilibre. Il entame alors une reconfiguration de proche en proche pour retrouver une stabilité. Or, dans un réseau complexe, la résolution de cette instabilité n'est pas nécessairement déterministe. La rupture d'une précondition peut en fragiliser plusieurs autres simultanément. L'infime asynchronisme dans l'ordre de chute des entités (la concurrence locale) crée des bifurcations. Ainsi, un même événement initial depuis $c$ ne mène pas inéluctablement à une unique configuration d'arrivée, mais peut ouvrir un faisceau de trajectoires divergentes vers plusieurs configurations valides. La dynamique de transition n'est donc pas une simple fonction, mais une \textbf{relation causale}.

\begin{definition}
	On appelle \textbf{chemin causal} partant d'une configuration valide $c$ vers une configuration valide $c'$ toute séquence \textbf{minimale} de transformations unitaires permettant au système de retrouver un état d'équilibre. En raison de l'asynchronisme local, il peut exister plusieurs chemins causaux distincts partant de $c$ suite à un même événement déclencheur.
\end{definition}

\subsection{La représentation canonique et la combinatoire pure}

Dans ce saut macroscopique, toute l'information relative à l'inertie du système et à l'ampleur de la cascade se résume à une seule métrique : la longueur de cette trajectoire.

\begin{definition}
	Pour un chemin causal donné reliant $c$ à $c'$, on appelle \textbf{coût d'édition} le nombre $d \in \mathbb{N}^*$ correspondant à la quantité exacte de transformations unitaires qui composent ce chemin.
\end{definition}

Il est fondamental d'insister sur la nature de ce coût : \textbf{il n'a aucune réalité énergétique ou thermodynamique}. Il ne s'agit pas d'un \og{}effort\fg{} continu à fournir. Il s'agit d'une métrique de \textbf{pure combinatoire}. C'est le décompte discret du volume d'événements (le nombre de \og{}dominos\fg{}) que la structure a mécaniquement imposés pour retomber sur ses pieds.

Cette lecture combinatoire offre une définition formelle et immédiate de l'instabilité :
\begin{itemize}
	\item Si $d(c, c') = 1$ : le chemin se résume exactement à une transformation unitaire. Le changement initial a été absorbé sans rompre aucune dépendance systémique.
	\item Si $d(c, c') > 1$ : le chemin causal est \textbf{composé}. L'événement initial a brisé la structure, forçant la mécanique locale à enchaîner $d-1$ événements secondaires pour retrouver un état valide. \textbf{C'est la définition mathématique et combinatoire de la défaillance en cascade.}
\end{itemize}

Dès lors, l'espace des possibles du système se formalise de manière extrêmement épurée sous la forme d'un objet mathématique classique :

\begin{definition}
	Pour un système donné, le multigraphe orienté et pondéré $G = (\mathcal{C}, \nu, d)$ s'appelle \textbf{la représentation canonique} du système. 
	\begin{itemize}
		\item \textbf{Les sommets} sont les configurations valides ($\mathcal{C}$).
		\item \textbf{Les arêtes} sont les chemins causaux minimaux entre ces configurations ($\nu$).
		\item \textbf{Le poids des arêtes} est le coût purement combinatoire du chemin ($d$).
	\end{itemize}
\end{definition}
...
\begin{table}[H]
	...
	\begin{tabular}{|l|l|l|}
		\hline 
		\textbf{Concept macroscopique} & \textbf{Symbole} & \textbf{Nature dans le graphe canonique} \\
		\hline 
		Configurations valides & $\mathcal{C}$ & Les sommets du graphe (les points d'équilibre). \\
		\hline 
		Chemins causaux & $\nu$ & Les arêtes (les relations asynchrones entre sommets). \\
		\hline 
		Distance d'édition & $d$ & Le poids des arêtes (volume combinatoire d'événements). \\
		\hline 
	\end{tabular}
	\caption{Résumé des éléments de la représentation canonique}
\end{table}

Cette vision macroscopique fige la dynamique du système en cartographiant l'univers de ses possibles. Elle permet de s'affranchir du bruit de fond des états transitoires et de bénéficier des méthodes d'analyse de graphes pour comprendre les dynamiques globales :

ATTRACTEURS, FLECHES DU TEMPS
