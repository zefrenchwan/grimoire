...
\begin{document}
	...
	Pour comprendre cette asymétrie sans recourir à un jugement de valeur externe (comme des lois physiques ou des nécessités sociales), il faut revenir à l'impact de la configuration sur la dynamique. Les entités, et tout particulièrement les liens, possèdent des \textbf{préconditions d'existence}. Un lien n'est maintenu que si la configuration locale — c'est-à-dire les entités qu'il connecte et leurs propriétés — le permet.

		
		\section{Le poids topologique d'un événement}
		
		C'est à la lumière de ce mécanisme formel que nous pouvons relire les dynamiques macroscopiques. Imaginons une configuration impliquant trois personnes : Paul, sa femme Laure, et leur fille Lisa. Comparons deux événements : \og{}Paul demande le divorce\fg{} et \og{}Paul donne une pièce à Lisa pour s'acheter une boisson\fg{}.
		
		Topologiquement, le lien \og{}mariage\fg{} est un nœud massivement intriqué : il sert de précondition à de très nombreux autres liens (domicile commun, autorité parentale conjointe, comptes bancaires partagés, etc.). Sa rupture invalide instantanément un pan entier de la configuration locale. L'obligation de retrouver un équilibre force une gigantesque cascade d'événements induits (déménager, séparer les biens). À l'inverse, l'événement \og{}donner une pièce\fg{} modifie une propriété locale (un solde) qui ne sert de précondition stricte à presque rien dans ce voisinage immédiat ; l'onde de choc s'amortit dès le premier événement.
		
		Pour caractériser objectivement le fait qu'un lien soit plus \og{}structurant\fg{} qu'un autre sans recourir à un jugement humain, nous pouvons évaluer le \og{}poids topologique\fg{} de l'événement qui le modifie. Deux mesures, intimement liées, s'offrent à nous : 
		
		\begin{enumerate}
			\item \textbf{La profondeur causale (impact dynamique)} : le nombre total d'événements induits en cascade par l'invalidation initiale, se propageant de proche en proche. L'onde de choc de la rupture conjugale provoque une vaste réorganisation (déménagements, séparation des biens) pour que le système retrouve une stabilité, tandis que l'achat de la boisson s'amortit immédiatement.
			\item \textbf{La variation de l'espace des possibles (impact topologique)} : la variation brute du nombre de configurations globalement accessibles au système avant et après l'événement. La disparition d'un lien fortement structurant condamne massivement des trajectoires de futurs (une baisse de l'entropie des possibles associés à ce lien) et en ouvre de nouvelles, redessinant instantanément le paysage des accessibilités. Le don d'une pièce de monnaie laisse, à l'inverse, l'horizon des possibles quasiment intact.
		\end{enumerate}
		
		
		\paragraph{Le relief causal et le principe de moindre action.} Si l'on reformule la dynamique du système en termes de réseau, il n'est pas nécessaire d'invoquer une \og{}énergie\fg{} abstraite pour expliquer pourquoi certaines transformations résistent. L'effort se mesure directement dans l'impact topologique : atteindre certaines configurations exige de briser des entités-liens structurantes et d'absorber une vaste cascade d'événements induits. Le \og{}coût\fg{} est en réalité une quantité de réécritures locales du graphe. La structure du système forme ainsi un véritable \og{}paysage\fg{}. Ses \og{}montagnes\fg{} correspondent aux zones où toute transformation exige une restructuration massive de l'espace des possibles. Ses \og{}vallées\fg{} correspondent aux transformations faciles, où il suffit de glisser le long des entités-liens préexistantes. Face à l'infinité des trajectoires théoriques, il s'impose alors un \textbf{principe de moindre action topologique} :
		
		\textit{La grande majorité du temps, la grande majorité des entités prend la pente et pas la montée. Elles empruntent les trajectoires qui minimisent la quantité d'événements restructurants.}
		
		Les trajectoires offrant le moins de résistance géométrique deviennent de véritables \og{}autoroutes causales\fg{}. Ce que l'observateur humain nomme parfois une \og{}tendance comportementale\fg{}, un biais de \og{}moindre effort\fg{} ou un choix, n'est bien souvent que l'observation statistique d'entités coulant le long des pentes de ce relief sous-jacent.
\end{document}