...
\begin{document}
	...
	\section{La notion de causalité}
	
		Prenons un exemple introductif : un chauffeur imprudent jette un mégot par la fenêtre, et celui-ci provoque un incendie de forêt très étendu. La chaine causale est claire : le mégot a déclenché un feu, le feu s'est propagé partout où la sécheresse des arbres permettait la propagation des flammes, jusqu'à disparaitre faute de ressources ou de zones favorables à proximité. Ainsi, une transformation a amorcé l'incendie depuis une configuration favorable, et il s'est propagé de proche en proche, parce que l'entité feu trouvait à proximité des arbres pour s'étendre. 
		
		
		Quel est, au delà de l'exemple, le mécanisme à l'œuvre pour qu'une entité soit présente dans une configuration ? Pour qu'un lien existe, il faut que les entités reliées soient présentes, c'est nécessaire. Pour qu'une entité soit présente et puisse tisser des liens avec d'autres, il faut que son voisinage soit déjà en place. Cette remarque induit une notion de causalité par propagation locale d'événements. 
	
		
		
		\begin{definition}
			Une précondition est une condition nécessaire pour qu'une entité soit présente dans une configuration. La \textbf{chaîne causale} initiée par un événement est la séquence ininterrompue d'événements induits, de proche en proche, par toute apparition ou disparition en cascade de préconditions.
		\end{definition}
...
\begin{definition}
	La \textbf{distance causale} (ou métrique topologique) entre deux configurations $a$ et $b$ correspond au \textbf{coût événementiel minimal}, c'est-à-dire au nombre total d'événements (initiés et induits en cascade) nécessaires pour transformer $a$ en $b$ tout en respectant la viabilité des préconditions à chaque étape.
\end{definition}

Cependant, toutes les entités ne sont pas égales face à cette métrique, car elles possèdent des préconditions d'existence et servent elles-mêmes de préconditions à d'autres.
...
L'événement \og{}donner une pièce\fg{} modifie une propriété locale (un solde) qui ne sert de précondition stricte à presque rien dans ce voisinage immédiat ; l'onde de choc s'amortit dès le premier événement. La distance causale pour absorber cette transformation est minimale.
À l'inverse, topologiquement, le lien \og{}mariage\fg{} est un nœud massivement intriqué : il sert de précondition à de très nombreux autres liens (domicile commun, autorité parentale conjointe, comptes bancaires partagés). Sa rupture invalide instantanément un pan entier de la configuration locale. L'obligation de retrouver un équilibre force une gigantesque cascade d'événements induits (déménagements, séparation des biens, etc.).

Nous pouvons ainsi définir l'équivalent topologique de la masse en physique :

\begin{definition}
	L'\textbf{inertie topologique} d'une entité (objet, propriété ou lien) est proportionnelle à l'étendue des chaînes causales dont elle constitue une précondition stricte. Elle caractérise la quantité d'événements qui seraient inévitablement induits par sa suppression ou son altération.
\end{definition}
