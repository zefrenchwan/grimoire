...
\begin{document}
	...
	\chapter{La dépendance causale}
		...
		Imaginons une configuration impliquant trois personnes : Paul, sa femme Laure, et leur fille Lisa. Comparons deux événements : \og{}Paul demande le divorce\fg{} et \og{}Paul donne un peu d'argent à Lisa pour s'acheter une boisson fraîche\fg{}. Dans une ontologie strictement plate, ces deux événements semblent de même nature : ils constituent l'un comme l'autre une simple création ou suppression locale de liens. Pourtant, nous percevons intuitivement qu'ils ne sont pas équivalents quant à leurs conséquences une fois tous les événements induits passés. Par exemple, la signature d'un divorce déclenche mécaniquement une série de nécessités légales et matérielles. L'un des ex-conjoints devra probablement déménager ; même s'il peut choisir son nouvel appartement parmi cinq options, cette obligation de relogement reste une conséquence directe de la rupture initiale. Ainsi, imposer que la chaîne causale détermine totalement et de manière unique toutes les configurations suivantes n'est pas la bonne approche. Il reste un système qui se réorganise, parfois avec une ambiguïté sur les détails, mais toujours sous la conséquence directe de la transformation initiale. 
		
		\begin{definition}
			La chaine causale d'une transformation $t$ est l'ensemble des événements qui engendrent $t$ ou qui sont la conséquence directe d'un événement précédemment dans cette même chaine causale. Dit autrement, pour une transformation $t$, sa chaine causale dure tant qu'il existe une contrainte forte sur l'espace des possibles. Elle dure tant que l'événement initial force le système à se réorganiser pour absorber l'anomalie topologique locale, et ce même s'il y a des bifurcations locales de choix. 
		\end{definition}
		
		Finalement, un lien a des préconditions d'application. Quand les entités changent autour de lui, ses préconditions sautent, le lien se supprime. Ailleurs, une précondition devient vraie : un lien se créé, ou une entité apparait. La chaine causale ne fait que traduire cette propagation d'impacts locaux qui peuvent, en fonction des entités impactées, rester sans effet notoire ou au contraire impacter drastiquement les configurations accessibles. Pour caractériser objectivement le fait qu'un lien soit plus \og{}structurant\fg{} qu'un autre sans recourir à un jugement de valeur externe, nous pouvons évaluer le \og{}poids topologique\fg{} de l'événement qui le modifie. Deux mesures, intimement liées, s'offrent à nous : 
		\begin{enumerate}
			\item \textbf{La profondeur causale (impact dynamique)} : le nombre total d'événements induits en cascade par la modification initiale, se propageant de proche en proche. L'onde de choc de la rupture conjugale provoque une vaste réorganisation (déménagements, séparation des biens) pour que le système retrouve une stabilité, tandis que l'achat de la boisson s'amortit immédiatement.
